...
public class MuteMenuCommand implements CommandExecutor {
    ...
    public MuteMenuCommand(WeGuardian plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (!staff.hasPermission("weguardian.mute")) {
            staff.sendMessage(MessageUtils.colorize("&cYou don't have permission to use this command."));
            return true;
        }

        if (args.length != 1) {
            staff.sendMessage(MessageUtils.colorize("&cUsage: /mutemenu <player>"));
            return true;
        }
        ...
        if (staff.getName().equalsIgnoreCase(targetPlayerName)) {
            staff.sendMessage(MessageUtils.colorize("&cYou cannot mute yourself!"));
            return true;
        }

        if (targetPlayer != null && targetPlayer.hasPermission("weguardian.bypass")) {
            staff.sendMessage(MessageUtils.colorize("&cYou cannot mute this player!"));
            return true;
        }

        plugin.getPunishmentGUI().getMenuManager().openMenu(staff, targetPlayerName, "mute");
        return true;
    }
