...
public class UnbanMenuCommand implements CommandExecutor, TabCompleter {

    private final WeGuardian plugin;

    public UnbanMenuCommand(WeGuardian plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (!staff.hasPermission("weguardian.unban")) {
            staff.sendMessage(MessageUtils.colorize("&cYou don't have permission to use this command."));
            return true;
        }

        if (args.length != 1) {
            staff.sendMessage(MessageUtils.colorize("&cUsage: /unbanmenu <player>"));
            return true;
        }
        ...
        plugin.getPunishmentGUI().getMenuManager().openMenu(staff, targetPlayerName, "unban");
        return true;
    }
