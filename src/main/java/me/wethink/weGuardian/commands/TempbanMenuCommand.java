...
public class TempbanMenuCommand implements CommandExecutor, TabCompleter {

    private final WeGuardian plugin;

    public TempbanMenuCommand(WeGuardian plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (!staff.hasPermission("weguardian.tempban")) {
            staff.sendMessage(MessageUtils.colorize("&cYou don't have permission to use this command."));
            return true;
        }

        if (args.length != 1) {
            staff.sendMessage(MessageUtils.colorize("&cUsage: /tempbanmenu <player>"));
            return true;
        }
        ...
        if (staff.getName().equalsIgnoreCase(targetPlayerName)) {
            staff.sendMessage(MessageUtils.colorize("&cYou cannot tempban yourself!"));
            return true;
        }

        if (targetPlayer != null && targetPlayer.hasPermission("weguardian.bypass")) {
            staff.sendMessage(MessageUtils.colorize("&cYou cannot tempban this player!"));
            return true;
        }

        plugin.getPunishmentGUI().getMenuManager().openMenu(staff, targetPlayerName, "tempban");
        return true;
    }
