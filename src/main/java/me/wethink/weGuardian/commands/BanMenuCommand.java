...
public class BanMenuCommand implements CommandExecutor, TabCompleter {

    private final WeGuardian plugin;

    public BanMenuCommand(WeGuardian plugin) {
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (!staff.hasPermission("weguardian.ban")) {
            staff.sendMessage(MessageUtils.colorize("&cYou don't have permission to use this command."));
            return true;
        }

        if (args.length != 1) {
            staff.sendMessage(MessageUtils.colorize("&cUsage: /banmenu <player>"));
            return true;
        }
        ...
        if (staff.getName().equalsIgnoreCase(targetPlayerName)) {
            staff.sendMessage(MessageUtils.colorize("&cYou cannot ban yourself!"));
            return true;
        }

        if (targetPlayer != null && targetPlayer.hasPermission("weguardian.bypass")) {
            staff.sendMessage(MessageUtils.colorize("&cYou cannot ban this player!"));
            return true;
        }

        plugin.getPunishmentGUI().getMenuManager().openMenu(staff, targetPlayerName, "ban");
        return true;
    }
