...
protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    ...
    out.println("        }");
    out.println("        function revokePunishment(id) { if(confirm('Are you sure you want to revoke this punishment?')) { fetch(`/api/punishments/${id}`, {method: 'DELETE'}).then(r=>r.json()).then(data=>{ if(data.success) { loadPunishments(); } else { alert('Error revoking punishment'); } }); } }");
