...
public class TempmuteMenu extends MenuHandler {
    ...
    protected ConfigurationSection getMenuConfig() {
        return plugin.getGUIConfigLoader().getGUIConfig("tempmute");
    }
        ...
        if (action.startsWith("execute_punishment:")) {
            ...
            if (parts.length >= 4) {
                ...
                if (selectedDuration != null && duration.contains("{selected_duration}")) {
                    ...
                    executeTempmute(staff, targetPlayer, reason, duration);
                    return;
                ...
                if (selectedDuration != null) {
                    executeTempmute(staff, targetPlayer, reason, selectedDuration);
                    return;
                } else {
                    staff.sendMessage(MessageUtils.colorize("&cPlease select a duration first!"));
                    return;
    ...
    private void executeTempmute(Player staff, String targetPlayer, String reason, String duration) {
        ...
        plugin.getPunishmentService().tempmute(targetPlayer, staff.getName(), reason, duration)
                .thenAccept(success -> {
                    if (success) {
                ...
                .exceptionally(throwable -> {
                    plugin.getLogger().severe("Error executing tempmute: " + throwable.getMessage());
